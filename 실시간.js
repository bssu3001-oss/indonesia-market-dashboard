...
(function () {
    ...
    const sets = await Promise.all(NEWS_FEEDS.map(async (f) => {
      try {
        ...
        if (xml) {
          ...
          items = [...doc.querySelectorAll('item')].slice(0, 12).map((item) => {
        ...
        if (!items.length) {
          const r = await fetch(`https://api.rss2json.com/v1/api.json?rss_url=${encodeURIComponent(f.url)}`, { signal: AbortSignal.timeout(10000) });
          if (r.ok) {
            const d = await r.json();
            items = (d.items || []).slice(0, 12).map((it) => ({
  ...
  function updateNewsBadgesFromKorean() {
    ...
    ko('badge-bi',
      ['금리 인하','완화','기준금리 내','BI 인하','인하 기대'],
      ['금리 인상','긴축','기준금리 올','BI 인상','인상 우려'],
      'BI 완화(호재)', 'BI 긴축(악재)', 'BI 정책 관망');
    ...
    ko('badge-trade',
      ['무역 협상','관세 완화','무역 합의','미-인도네시아 협정','수출 증가'],
      ['관세 부과','무역 갈등','무역 제재','수출 감소','관세 위협'],
      '무역 호조', '무역 리스크', '무역 주시');

    ko('badge-commodity',
      ['석탄 상승','팜유 상승','니켈 상승','석탄 강세','팜유 강세','원자재 상승'],
      ['석탄 하락','팜유 하락','니켈 하락','석탄 약세','팜유 약세','원자재 하락'],
